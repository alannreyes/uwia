...
export class UnderwritingController {
  ...
  async evaluateClaim(
    ...
    if (file) {
      const largeFileThresholdMB = this.configService.get<number>('LARGE_FILE_THRESHOLD_MB') || 20;  // Google official Files API threshold
  ...
  @UseInterceptors(FileInterceptor('file')) // Single file per call (N8N actual behavior)
  @HttpCode(HttpStatus.OK)
